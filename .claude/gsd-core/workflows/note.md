...
- **Global scope**: `/Users/noobian/dev/openblueprint/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/noobian/dev/openblueprint/.claude/notes/`)
...
2. Glob `/Users/noobian/dev/openblueprint/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/noobian/dev/openblueprint/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/noobian/dev/openblueprint/.claude/notes/` — works in any directory
